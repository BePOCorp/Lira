...
//        $this->db->join("tb_actividad a","cnl.idtb_actividad=a.idtb_actividad");
        $this->db->select("a.descripcion actividad, sem.descripcion semana, pt.descripcion plan_de_trabajo, pt.cumplimiento");
        ...
        $this->db->join("docente doc", "cnl.id_docente = doc.id");
...
//        return $this->db->insert("tb_plan_de_trabajo", $data);
        return $this->db->insert("plan_de_trabajo", $data);
    }
...
//        $this->db->where("idtb_actividad", $valor);
        $this->db->select("id");
        $this->db->from("carga_no_lectiva");
        $this->db->where("id_actividad", $valor);
        $resultados = $this->db->get();
        return $resultados->result();
    }
...
//        $this->db->where("usu.idtb_usuario", $this->session->userdata("idtb_usuario"));
        $this->db->select("act.id id_actividad, act.descripcion");
        $this->db->from("actividad act");
//        $this->db->join("carga_no_lectiva asi", "act.id = asi.id_actividad");
        //$this->db->join("docente doc", "cnl.id_docente = doc.id");
//        $this->db->where("doc.id", $this->session->userdata("id"));
        $resultados = $this->db->get();
        return $resultados->result();
